...
int parse( n_ad, m_ad, nodes_ad, arcs_ad, 
           source_ad, node_min_ad, problem_name, maxlen_ad, spfile )
...
#define MAXLINE       128	/* max line length in the input file */
...
while ( ret = fgets (in_line, MAXLINE, fpsp ) != NULL && ret != EOF)
  {
  no_lines ++;
...
//if ( feof (stdin) == 0 ) /* reading error */	// Not needed when using fgets()

...
// use_average = false
//     all vessel nodes are in the range
//-----------------------------------------------------------------------------------------------------
...
int main(int argc, char **argv)
{
	...
	int ave_flag, cmgui_flag;
	float diam_min, diam_max, origin_shift[3];
	bool use_average;
	NETWORK *NP0, *NP1;

	if (argc != 7) {
		printf("Usage: select input_amfile output_amfile diam_min diam_max ave_flag cmgui_flag\n");
		fperr = fopen("select_error.log","w");
		fprintf(fperr,"Usage: select input_amfile output_amfile diam_min diam_max ave_flag cmgui_flag\n");

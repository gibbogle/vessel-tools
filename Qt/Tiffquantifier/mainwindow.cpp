    ...
    ui(new Ui::MainWindow)
{
    ...
    QString infoFile = QCoreApplication::applicationDirPath() + "/info/tiffquantifier_info.txt";
	QFile file(infoFile);
...
void MainWindow::checkReady()
{
    ...
    if (voxelsize[0] > 0 && voxelsize[1] > 0 && voxelsize[2] > 0)
        ...
        voxelOK = false;
    if ((voxelOK || isSetup()) && is_tifffile && is_closefile && is_resultfile) {
        is_ready = true;
...
//        ui->lineEdit_mm_mm3->clear();
        ...
        islice = ui->lineEdit_intercept->text().toInt();    // 1-based!
        fprintf(fpout,"\n------------------------------------------------------------------------------\n");
...
void MainWindow::doSetup()
{
    ...
    char input_tifffile[1024], close_file[1024], result_file[1024];

    this->setCursor( QCursor( Qt::WaitCursor ) );
    ui->labelResult->setText("Doing setup()");
    strcpy(input_tifffile, tiffFileName.toAscii().constData());
    ...
    if (!isSetup()) {
        err = setup(input_tifffile,close_file,result_file,voxelsize);

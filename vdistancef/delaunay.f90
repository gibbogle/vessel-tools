...
!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
subroutine test_delaunay
use par_zig_mod
integer :: ip, np, ntri, kpar = 0
real ( kind = 8 ) :: v(2,3)
real ( kind = 8 ), allocatable, dimension ( :, : ) :: table
...
if (generate) then
    ...
    allocate(table(2,np))
    do ip = 1,np
        table(1,ip) = par_uni(kpar)
        table(2,ip) = par_uni(kpar)
    enddo
else        
    ...
    allocate(table(2,np))
    do ip = 1,np
        read(10,*) table(1,ip), table(2,ip)
    enddo
    close(10)
endif
allocate ( triangle_node(triangle_order,3*np) )     ! node numbers for the triangles
call triangulate(np, table, ntri, triangle_node)
!call total_area(np, table, ntri, triangle_node)
deallocate ( table )
...
v(1,3) = 0
v(2,3) = 1
write(*,*) 'Area: ',triangle_area(v)
end subroutine

!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
subroutine total_area(np, table, ntri, triangle_node, area)
integer :: np, ntri
real(kind=8) :: table(:,:)
integer :: triangle_node(:,:)
real :: area
integer :: i, j, nd
real(kind=8) :: v(2,3)
...
do i  = 1,ntri
    do j = 1,3
        nd = triangle_node(j,i)
        v(1,j) = table(1,nd)
        v(2,j) = table(2,nd)
    enddo
    area = area + triangle_area(v)
enddo
end subroutine

!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
real function triangle_area(v)
real(kind=8) :: v(2,3)

triangle_area = abs(v(1,1)*(v(2,2)-v(2,3)) + v(1,2)*(v(2,3)-v(2,1)) + v(1,3)*(v(2,1)-v(2,2)))/2
end function

!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
subroutine triangulate(node_num, table, triangle_num, triangle_node)
integer :: node_num
real(kind=8) :: table(:,:)
...
!  call file_name_ext_swap ( eps_file_name, 'delaunay.eps' )
  ...
  call triangulation_plot_eps ( 'delaunay.eps', node_num, table(1,1:node_num), &
...
!
integer function diaedg_lmap ( x0, y0, x1, y1, x2, y2, x3, y3, matrix )
  implicit none
...
!    this happens.
!
integer function s_index_last ( s, sub )
  implicit none

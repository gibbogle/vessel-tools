...
extern float ddiam, dlen, dminimum;
...
//			printf("ip,kp: %4d %4d x,y,z,d: %6.1f %6.1f %6.1f %6.1f\n",ip,kp,points[kp].x,points[kp].y,points[kp].z,points[kp].d);
			if (ad < 0.001 || ad > 100) {
				printf("Bad point diameter: edge: %d point: %d %d ad: %f\n",ie,ip,kp,ad);
				fprintf(fperr,"Bad point diameter: edge: %d point: %d %d ad: %f\n",ie,ip,kp,ad);
				return 1;
			}
			if (ip > 0) {
...
//		fprintf(fpout,"ie: %6d dave,len,ave_lseg_diam: %8.1f %8.1f %10.1f\n",ie,dave,len,ave_lseg_diam);
	...
	printf("\nCompute length distributions: dlen: %f NBOX: %d\n",dlen,NBOX);
